...
K0, QN = DiscreteTimeLinearQuadraticRegulator(A0,B*h,Q,R)
...
u = np.full((N, m), -1)
for t in range(N):
#    x[t+1] = A0.dot(x[t]) + B0.dot(u[t])
    x[t+1] = x[t] + h*(A.dot(x[t]) + B.dot(u[t]))
...
for j in range(1):
    ...
    for t in range(N):
        ...
        x_new[t+1] = x_new[t] + h*(A.dot(x_new[t]) + B.dot(u_new[t]))
#        x_new[t+1] = A0.dot(x_new[t]) + B0.dot(u_new[t])
